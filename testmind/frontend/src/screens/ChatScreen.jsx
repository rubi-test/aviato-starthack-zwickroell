...
function ProgressBar({ active }) {
  ...
  return (
    <div className="absolute top-0 left-0 right-0 h-0.5 bg-slate-200 overflow-hidden z-50">
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-50 overflow-hidden relative">
      ...
      <header className="bg-white border-b border-slate-200 px-5 py-2.5 flex items-center gap-3 flex-shrink-0">
        <button onClick={onBack} className="text-xs text-blue-600 hover:text-blue-500 transition-colors flex items-center gap-1 font-mono">
          ← BACK
        </button>
        <span className="text-slate-700">|</span>
        <span className="text-xs font-semibold text-slate-500 font-mono uppercase tracking-wider">TestMind Chat</span>
        {contextInfo && (
          <div className="flex items-center gap-1.5 ml-2">
            {contextInfo.map((tag, i) => (
              <span key={i} className="text-[10px] bg-blue-50 text-blue-600 px-2 py-0.5 rounded border border-blue-200 font-mono">
                {tag}
              </span>
            ))}
          </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {messages.length > 0 && (
            <button
              ...
              className="text-[10px] text-slate-500 hover:text-slate-900 border border-slate-200 hover:border-slate-500 rounded px-2 py-1 transition-colors flex items-center gap-1 font-mono"
          ...
          {messages.length > 1 && (
            <button
              ...
              className="text-[10px] text-slate-500 hover:text-red-600 border border-slate-200 hover:border-red-700 rounded px-2 py-1 transition-colors font-mono"
              title="Clear conversation"
            >
              CLEAR
            </button>
          )}
          <span className="text-[10px] text-slate-500 bg-slate-100 px-2.5 py-1 rounded font-mono border border-slate-200">ULM</span>
        </div>
      </header>
