...
const QUICK_PROMPTS = [
  "Line chart of tensile strength over time for FancyPlast 42",
  "Bar chart comparing tensile strength across all materials",
  "Scatter plot of tensile strength vs elongation for UltraPlast 99",
  "Histogram of tensile modulus distribution",
  "Pie chart of tests by material",
  "Compare FancyPlast 42 and Hostacomp G2 tensile modulus over time",
];
    ...
    return (
      <ResponsiveContainer width="100%" height={380}>
        <LineChart data={merged} margin={{ top: 10, right: 30, left: 10, bottom: 5 }}>
          ...
          <Tooltip contentStyle={{ background: "#1e2433", border: "1px solid #2a3144", borderRadius: 8, fontSize: 12, color: "#cbd5e1" }} />
...
export default function GraphBuilderScreen({ onBack }) {
  ...
  const handleExportPNG = () => {
    ...
    img.onload = () => {
      ...
      ctx.scale(2, 2); ctx.fillStyle = "#1e2433"; ctx.fillRect(0, 0, canvas.width, canvas.height);
  ...
  return (
    <div className="h-screen flex flex-col bg-[#0f1117] overflow-hidden">
      ...
      <header className="bg-[#141820] border-b border-[#1e2433] px-5 py-2.5 flex items-center gap-3 flex-shrink-0">
        <button onClick={onBack} className="text-xs text-blue-400 hover:text-blue-300 transition-colors font-mono">← BACK</button>
        <span className="text-slate-700">|</span>
        <span className="text-xs font-semibold text-slate-400 font-mono uppercase tracking-wider">Graph Builder</span>
        <div className="flex items-center gap-1.5 ml-2">
          <span className="text-[10px] bg-purple-900/30 text-purple-400 px-2 py-0.5 rounded border border-purple-800/40 font-mono">AI-POWERED</span>
        </div>
        <div className="flex-1" />
        {currentResult && (
          <div className="flex items-center gap-2">
            <button onClick={handleExportPNG} className="text-[10px] text-slate-500 hover:text-slate-300 border border-slate-700 hover:border-slate-500 rounded px-2 py-1 font-mono transition-colors">↓ PNG</button>
            <button onClick={handleExportSVG} className="text-[10px] text-slate-500 hover:text-slate-300 border border-slate-700 hover:border-slate-500 rounded px-2 py-1 font-mono transition-colors">↓ SVG</button>
          </div>
        )}
      </header>
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Left: Chat panel (35%) */}
        <div className="w-[35%] border-r border-[#1e2433] flex flex-col">
          <div className="flex-1 overflow-y-auto p-4 space-y-3 thin-scrollbar">
            ...
            {messages.length === 0 && (
              <div className="text-center pt-4 space-y-4">
                <div className="text-slate-500 text-xs font-mono space-y-1">
                  <p className="text-slate-300 text-sm font-semibold mb-3">Build custom graphs with AI</p>
                  <p>Describe the chart you want in plain English.</p>
                  <p>I'll build it or explain what won't work.</p>
                </div>
                <div className="space-y-1.5 mt-4">
                  <p className="text-[10px] text-slate-600 font-mono uppercase tracking-wider">Quick starts</p>
                  {QUICK_PROMPTS.map((q, i) => (
                    <button key={i} onClick={() => handleSend(q)}
                      className="w-full text-left text-xs text-slate-400 hover:text-slate-200 bg-slate-800/50 hover:bg-slate-800 border border-slate-700 rounded px-3 py-2 transition-colors font-mono truncate">
                      {q}
                    </button>
                  ))}
                </div>
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[90%] px-3 py-2 rounded-lg text-sm ${
                  ...
                  <p>{msg.content}</p>
                  {msg.explanation && (
                    <p className="text-xs text-blue-300/60 mt-1 font-mono">{msg.explanation}</p>
                  )}
                  {msg.suggestions && (
                    <div className="mt-2 space-y-1">
                      {msg.suggestions.map((s, j) => (
                        <p key={j} className="text-xs text-amber-400/70 font-mono">• {s}</p>
                      ))}
                    </div>
                  )}
                </div>
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="bg-[#1e2433] border border-[#2a3144] px-3 py-2 rounded-lg text-xs text-slate-400 font-mono flex items-center gap-2">
          ...
          {/* Input */}
          <div className="p-3 border-t border-[#1e2433] bg-[#141820]">
            <div className="flex gap-2">
              <input
                ...
                disabled={isLoading}
                className="flex-1 px-3 py-2 border border-slate-700 rounded-lg text-sm bg-[#1e2433] text-slate-200 placeholder:text-slate-600 focus:outline-none focus:ring-1 focus:ring-blue-500 disabled:opacity-50 font-mono"
              />
              <button
                onClick={() => handleSend()}
                disabled={isLoading || !input.trim()}
                className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-mono hover:bg-blue-500 disabled:bg-slate-700 disabled:text-slate-500 transition-colors"
              >
                BUILD
              </button>
            </div>
        ...
        <div className="flex-1 flex flex-col bg-[#0f1117] grid-pattern">
          {!currentResult ? (
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center space-y-3">
                ...
                <p className="text-xs text-slate-600 font-mono">Describe what you want in the chat</p>
              </div>
            </div>
          ) : (
            <div className="flex-1 flex flex-col p-6 overflow-y-auto">
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h2 className="text-lg font-semibold text-slate-200 font-mono">{currentResult.title}</h2>
                  <p className="text-xs text-slate-500 font-mono mt-0.5">{currentResult.explanation}</p>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] bg-slate-800 text-slate-400 px-2 py-1 rounded border border-slate-700 font-mono uppercase">{currentResult.chart_type}</span>
                </div>
              </div>
              ...
              {currentResult.series && (
                <div className="mt-4 flex flex-wrap gap-2">
                  {currentResult.series.map((s, i) => (
                    <div key={i} className="inline-flex items-center gap-2 bg-slate-800/50 border border-slate-700 rounded px-3 py-1.5">
                      <span className="w-2 h-2 rounded-full" style={{ backgroundColor: COLORS[i % COLORS.length] }} />
                      <span className="text-xs text-slate-400 font-mono">{s.name}</span>
                      <span className="text-[10px] text-slate-600 font-mono">{s.data?.length || 0} pts</span>
                    </div>
                  ))}
                </div>
              )}
              ...
              {graphSpec && (
                <div className="mt-4 bg-[#141820] border border-[#2a3144] rounded-lg p-3 animate-fadeIn">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    <span className="text-[10px] bg-blue-900/30 text-blue-400 px-2 py-0.5 rounded border border-blue-800/40 font-mono">{graphSpec.chart_type}</span>
                    {graphSpec.materials?.map((m) => (
                      <span key={m} className="text-[10px] bg-emerald-900/30 text-emerald-400 px-2 py-0.5 rounded border border-emerald-800/40 font-mono">{m}</span>
                    ))}
                    {graphSpec.properties?.map((p) => (
                      <span key={p} className="text-[10px] bg-purple-900/30 text-purple-400 px-2 py-0.5 rounded border border-purple-800/40 font-mono">{p.replace(/_/g, " ")}</span>
                    ))}
                  </div>
                  <p className="text-[10px] text-slate-600 font-mono mt-2">Say "add FancyPlast 84" or "remove UltraPlast 99" to modify</p>
                </div>
              )}
            </div>
          )}
