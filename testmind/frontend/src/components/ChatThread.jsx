...
export default function ChatThread({ messages, isLoading, onSend, disabled, onBookmark, savedQueries = [], onScrollToResult }) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        {messages.map((msg, i) => {
          // Compute which response index this assistant message corresponds to
          const respIndex = msg.role === "assistant"
            ? messages.slice(0, i).filter((m) => m.role === "assistant").length
            : null;
                ...
                >
                  {msg.role === "assistant" ? (
                    <div>
                      ...
                      <button
                        onClick={() => onScrollToResult?.(respIndex)}
                        className="text-xs text-blue-500 hover:text-blue-700 mt-1.5 text-left transition-colors"
                      >
                        See full details in the results panel →
                      </button>
                    </div>
                  ) : (
                    msg.content
