    ...
    >
      ...
      {chart_data && (
        <div className="flex flex-wrap gap-2 animate-fadeIn">
          {chart_data.material && (
            <span className="text-[10px] bg-blue-900/30 text-blue-400 px-2.5 py-1 rounded border border-blue-800/40 font-mono">{chart_data.material}</span>
          )}
          {chart_data.trend_direction && (
            <span className={`text-[10px] px-2.5 py-1 rounded border font-mono ${
              chart_data.trend_direction === "decreasing" ? "bg-red-900/30 text-red-400 border-red-800/40" :
              chart_data.trend_direction === "increasing" ? "bg-emerald-900/30 text-emerald-400 border-emerald-800/40" :
          ...
          {chart_data.pass_rate_pct != null && (
            <span className={`text-[10px] px-2.5 py-1 rounded border font-mono font-semibold ${
              ...
              chart_data.pass_rate_pct >= 80 ? "bg-amber-900/30 text-amber-400 border-amber-800/40" :
          ...
          {chart_data.correlation_coefficient != null && (
            <span className="text-[10px] bg-indigo-900/30 text-indigo-400 px-2.5 py-1 rounded border border-indigo-800/40 font-mono">r={chart_data.correlation_coefficient}</span>
          )}
          {chart_data.will_violate != null && (
            <span className={`text-[10px] px-2.5 py-1 rounded border font-mono font-semibold ${
              chart_data.will_violate ? "bg-red-900/30 text-red-400 border-red-800/40" : "bg-emerald-900/30 text-emerald-400 border-emerald-800/40"
            }`}>
              {chart_data.will_violate ? "VIOLATION PREDICTED" : "WITHIN BOUNDS"}
            </span>
          )}
          {chart_data.count != null && (
            <span className="text-[10px] bg-slate-800 text-slate-400 px-2.5 py-1 rounded border border-slate-700 font-mono">{chart_data.count} results</span>
          )}
        </div>
      )}
          ...
          <div className="flex flex-col gap-1 flex-shrink-0">
            {answer && (
              <button onClick={() => { navigator.clipboard.writeText(answer).then(() => addToast("Copied", "success")); }}
                className="flex items-center gap-1 text-[10px] text-slate-500 hover:text-slate-300 border border-slate-700 hover:border-slate-500 rounded px-2 py-1 transition-colors font-mono">
                COPY
              </button>
            )}
      ...
      {suggested_followups?.length > 0 && (
        <div className="bg-blue-950/20 border border-blue-800/30 rounded-lg p-4">
          <p className="text-[10px] font-semibold text-blue-400 uppercase tracking-widest mb-2 font-mono">Suggested Next</p>
          <div className="flex flex-col gap-2">
            {suggested_followups.map((q, i) => (
              <button key={i} onClick={() => onFollowUp && onFollowUp(q)}
                className="text-left text-sm text-blue-300/80 hover:text-blue-200 hover:bg-blue-900/20 rounded-lg px-3 py-2 border border-blue-800/30 hover:border-blue-600/40 transition-all hover-lift font-mono">
                → {q}
              </button>
            ))}
          </div>
